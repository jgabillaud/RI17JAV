...
//Manipulation d'un stream générique
		...
		//Transformer une collection de personnes en collection de records
		System.out.println("Création d'une liste de personnes simplifiées à partir de la liste de personnes:");
		record PersonneSimplifiee(String nom, String prenom) {};
		List<PersonneSimplifiee> listePersonnesSimplifiees = listePersonnes.stream().map(p->new PersonneSimplifiee(p.getNom(), 
																										p.getPrenom()))
																		.collect(Collectors.toList());
		listePersonnesSimplifiees.forEach(c->System.out.println(c));
